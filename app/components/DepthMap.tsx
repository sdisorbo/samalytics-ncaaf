...
const bands = Array.from({ length: (Y_MAX - Y_MIN) / STEP }, (_, i) => Y_MIN + i * STEP);
...
export default function DepthMap({ id, name, position, seasons }:
  ...
  const typeLabel: Record<string, string> = { pass: "Passing", rush: isQB ? "Scrambles" : "Rushing", rec: "Receiving", both: "Both" };
  ...
  useEffect(() => {
    if (!year) return;
    setPlays(null);
    fetch(`/api/player/plays?n=${encodeURIComponent(name)}&year=${year}&team=${encodeURIComponent(teamOf(year))}&pos=${encodeURIComponent(position)}`)
      .then((r) => r.json()).then((d) => setPlays(d.plays || [])).catch(() => setPlays([]));
  }, [year, name, position]); // eslint-disable-line react-hooks/exhaustive-deps

  const shown = useMemo(() => {
    const all = plays || [];
    if (typeOpts.length === 1 || typeSel === "both") return all;
    return all.filter((p) => p.type === typeSel);
  }, [plays, typeSel, typeOpts.length]);
  ...
  const heat = (i: number): string => {
    ...
    if (metric === "epa") { const a = stats.avgEpa[i]; return a >= 0 ? `rgba(214,73,91,${(0.12 + 0.68 * a / stats.maxAbsEpa).toFixed(3)})` : `rgba(80,110,190,${(0.12 + 0.55 * -a / stats.maxAbsEpa).toFixed(3)})`; }
    const t = metric === "volume" ? s.n / stats.maxN : Math.abs(stats.avgYds[i]) / stats.maxYds;
    ...
    <div className="mt-7">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        {typeOpts.length > 1 && (
          <div className="segment">
            {typeOpts.map((t) => (
              <button key={t} className={typeSel === t ? "on" : ""} onClick={() => setTypeSel(t)}>{typeLabel[t]}</button>
            ))}
          </div>
        )}
      </div>

      <div className="grid md:grid-cols-[150px_1fr] gap-5 items-start">
          ...
          <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} className="rounded-lg block" style={{ background: "#2f7a43" }}>
            ...
            {Array.from({ length: 7 }, (_, k) => k * 10).map((v) => (
              <g key={v}>
                ...
                <text x={4} y={yPix(v) - 3} fontSize={8} fill="rgba(255,255,255,0.7)">{v === 0 ? "" : `+${v}`}</text>
              </g>
            ))}
            <line x1={0} x2={W} y1={yPix(0)} y2={yPix(0)} stroke="#2b6cff" strokeWidth={3} />
            <text x={W - 4} y={yPix(0) - 4} fontSize={9} fill="#bcd2ff" textAnchor="end" fontWeight="bold">LOS</text>
            <line x1={0} x2={W} y1={yPix(10)} y2={yPix(10)} stroke="#ffd21e" strokeWidth={2.5} />
            <text x={W - 4} y={yPix(10) - 4} fontSize={9} fill="#ffe98a" textAnchor="end" fontWeight="bold">1st down</text>
            {shown.map((p, i) => (
              <circle key={i} cx={W / 2 + (hash(i) - 0.5) * W * 0.5} cy={yPix(p.yards)} r={p.td ? 4 : 3}
                fill={dotColor(p)} fillOpacity={0.85} stroke={p.td ? "#ffd21e" : "rgba(0,0,0,0.35)"} strokeWidth={p.td ? 1.5 : 0.6} />
            ))}
            {plays == null && <text x={W / 2} y={H / 2} fontSize={11} fill="#fff" textAnchor="middle">loading…</text>}
          </svg>
          <div className="flex justify-center flex-wrap gap-x-3 mt-1.5 text-2xs text-s-muted">
            {isQB ? <><Dot c="#ffffff" /> completion {typeSel !== "pass" && <><Dot c="#111827" /> scramble</>}</>
              : isRB ? <><Dot c="#ffffff" /> catch <Dot c="#111827" /> carry</>
                : <><Dot c="#ffffff" /> catch</>}
            <span>· gold = TD</span>
          </div>
        </div>
        ...
        <div className="text-sm">
          ...
          <p className="text-2xs text-s-muted mt-3 leading-relaxed">
            Each dot is a real {isQB ? "completion" : isRB ? "carry or catch" : "catch"} placed by its actual yards gained
            past the line of scrimmage (throw + YAC). The field is intentionally narrow: horizontal position is not
            tracked in college data, so a dot&apos;s side means nothing. Live from the CollegeFootballData API.
          </p>
        </div>
      </div>
    </div>
